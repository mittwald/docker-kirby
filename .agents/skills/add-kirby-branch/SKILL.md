...
That workflow checks weekly for a Kirby major nothing builds yet, and when it
finds one it runs this skill under opencode on a branch of its own. So the
usual caller is not a person: it is an unattended job that expects a finished,
verified pull request, or a clear statement that nothing was done.

The run arrives with a briefing generated by `scripts/check-updates.py`,
carrying the facts you would otherwise go and gather — the best PHP version,
whether a matching plainkit release exists and which constraint to use if not,
and which branch currently holds `latest`. Trust it and skip re-deriving them.

The workflow applies PHP bumps for existing branches on its own; those arrive
as an ordinary pull request and need nothing from this skill.
